...
Provides:
  ...
  load_arxiv768()    : arXiv abstracts InstructorXL-768, 100k corpus, 2k queries (cached)
...
def load_arxiv768() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """arXiv abstracts InstructorXL-768: 100k corpus, 2k queries, cached ground truth.

    Source: ``Qdrant/arxiv-abstracts-instructorxl-embeddings`` (2.25M rows, train split).
    Embeddings are 768-dim unit-normalised vectors generated by InstructorXL.
    First 2k rows become queries; next 100k rows become the corpus.

    Returns:
        vecs:      ``(100_000, 768)`` float32, unit-normalised
        qvecs:     ``(2_000,   768)`` float32, unit-normalised
        true_top1: ``(2_000,)``      int64, corpus index of true nearest neighbour
    """
    os.makedirs(CKPT_DIR, exist_ok=True)
    N_DATA, N_QUERIES = 100_000, 2_000
    tag = "arxiv768"
    ckpt_vecs  = CKPT_DIR / f"{tag}_{N_DATA}_vecs.npy"
    ckpt_qvecs = CKPT_DIR / f"{tag}_{N_QUERIES}_qvecs.npy"
    ckpt_truth = CKPT_DIR / f"{tag}_{N_DATA}_truth_top1.npy"
    ...
    if ckpt_vecs.exists() and ckpt_qvecs.exists():
        print("  Loading cached arXiv-768 vectors ...", flush=True)
        vecs  = np.load(ckpt_vecs)
        qvecs = np.load(ckpt_qvecs)
    else:
        from datasets import load_dataset
        HF = "Qdrant/arxiv-abstracts-instructorxl-embeddings"
        print(f"  Downloading arXiv-768 from {HF} ...", flush=True)
        ...
        for i, row in enumerate(ds):
            ...
            if (i + 1) % 10_000 == 0:
                print(f"    {i+1:>7,} / {N_DATA + N_QUERIES:,}", flush=True)
        all_vecs = np.array(raw, dtype=np.float32)
        # Normalize (InstructorXL outputs unit vectors, but guard against fp drift)
        norms = np.linalg.norm(all_vecs, axis=1, keepdims=True)
        all_vecs /= np.where(norms > 0, norms, 1.0)
        qvecs = all_vecs[:N_QUERIES]
        vecs  = all_vecs[N_QUERIES:]
    ...
    if ckpt_truth.exists():
        print("  Loading cached arXiv-768 ground truth ...", flush=True)
        true_top1 = np.load(ckpt_truth)
    else:
        print(f"  Computing arXiv-768 ground truth ({N_QUERIES} queries) ...", flush=True)
        t0 = time.perf_counter()
        rows = [np.argmax(qvecs[i:i+50] @ vecs.T, axis=1) for i in range(0, N_QUERIES, 50)]
        true_top1 = np.concatenate(rows)
        print(f"  Done in {time.perf_counter() - t0:.1f}s", flush=True)
    ...
    print(f"  arXiv-768: corpus={vecs.shape}, queries={qvecs.shape}", flush=True)
    return vecs, qvecs, true_top1
